export async function shuffleDeck(){
    try{
        let __deck = await fetch('https://deckofcardsapi.com/api/deck/new/shuffle/?deck_count=1').then(response => response.json());
...
export async function drawHand(__deck,__number){
    ...
    for (let i = 0; i < __number; i++){
        const card = fetch(`https://deckofcardsapi.com/api/deck/${deckId}/draw/?count=1`).then(response => response.json());
        cards.push(card);
    }
